...
def _render_sidebar_new():
    ...
    with st.expander("📖 הסבר מתמטי על המודל"):
        model_tab1, model_tab2 = st.tabs(["📈 עם גידול (11%)", "📊 ללא גידול (17%)"])
        
        with model_tab1:
            st.markdown("""
### מודל 11% – עם גידול אוכלוסייה

---

#### 🎯 הבסיס הדמוגרפי

מודל 11% מבוסס על **סטטיסטיקה דמוגרפית** ארוכת טווח.
...
**בסיס (ללא גידול):** 42.6% × 0.4 = **17%**

**עם גידול 5%:**
- הגידול מוסיף משפחות צעירות
- מדלל את אחוז הלווים
- **מתייצב על ~11%**
...
- **עמידות** – הגידול מחזק את המודל
- **מוכח** – גמח"ים ותיקים משתמשים בו עשרות שנים
            """)
        
        with model_tab2:
            st.markdown("""
### מודל 17% – ללא גידול אוכלוסייה

---

#### 🎯 אוכלוסייה יציבה

כשמספר המצטרפים **קבוע** בכל שנה, האוכלוסייה 
מגיעה ל**שיווי משקל** מושלם אחרי ~50 שנה.

הפירמידה הדמוגרפית הופכת ל**מלבן אחיד**.
...
| תקופת חתונות | 20 שנה |
| תקופת חברות | 47 שנה |
| אחוז בחתונות | 20÷47 = **42.55%** |
| הלוואות/שנה | 0.4 |
| **אחוז לווים** | 42.55% × 0.4 = **17.02%** |

---

#### ⚖️ השוואה בין המודלים

| מאפיין | עם גידול | ללא גידול |
|--------|----------|-----------|
| פירמידה | משולש | מלבן |
| % בחתונות | נמוך | 42.6% |
| **% לווים** | **11%** | **17%** |
...
- יציאה שנתית: 17% × 100,000 = **17,000 ₪**
- הכנסה: דמי חברות + החזרים
- **גירעון זמני** בתקופת בנייה
- **יתרה חיובית** לאחר שיווי משקל

---

#### ✅ יתרונות

- **פשטות קיצונית** – חישובים מדויקים
- **שמרנות מירבית** – גם בסטטי מאוזן
- **הוכחה** – אם עובד בלי גידול, 
  בהכרח יעבוד טוב יותר עם גידול!
            """)
